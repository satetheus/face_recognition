...
try:
    from face_recognition_models import (
        pose_predictor_model_location as predictor_68_point_model,
        pose_predictor_five_point_model_location as predictor_5_point_model,
        cnn_face_detector_model_location as cnn_face_detection_model,
        face_recognition_model_location as face_recognition_model,
    )
except Exception:
...
def _rect_to_css(rect):
    ...
    :return: a plain tuple representation of the rect in (top, right, bottom,
        left) order
    """
...
def _css_to_rect(css):
    ...
    :param css:  plain tuple representation of the rect in (top, right, bottom,
        left) order
...
def _trim_css_to_bounds(css, image_shape):
    """
    Make sure a tuple in (top, right, bottom, left) order is within the bounds
    of the image.

    :param css:  plain tuple representation of the rect in (top, right, bottom,
        left) order
    :param image_shape: numpy shape of the image array
    :return: a trimmed plain tuple representation of the rect in (top, right,
        bottom, left) order
    """
    return (
        max(css[0], 0),
...
def load_image_file(file, mode='RGB'):
    ...
    :param mode: format to convert the image to. Only 'RGB' (8-bit RGB, 3
        channels) and 'L' (black and white) are supported.
...
def _raw_face_locations(img, number_of_times_to_upsample=1, model="hog"):
    ...
    :param model: Which face detection model to use. "hog" is less accurate but
        faster on CPUs. "cnn" is a more accurate deep-learning model which is
...
def face_locations(img, number_of_times_to_upsample=1, model="hog"):
    ...
    :param model: Which face detection model to use. "hog" is less accurate but
        faster on CPUs. "cnn" is a more accurate deep-learning model which is
        GPU/CUDA accelerated (if available). The default is "hog".
    :return: A list of tuples of found face locations in css (top, right,
        bottom, left) order
    """
    if model == "cnn":
        return [
...
def batch_face_locations(images, number_of_times_to_upsample=1, batch_size=128):
    """
    Returns an 2d array of bounding boxes of human faces in a image using the
    cnn face detector. If you are using a GPU, this can give you much faster
    results since the GPU can process batches of images at once. If you aren't
...
def face_landmarks(face_image, face_locations=None, model="large"):
    ...
    if model == 'large':
        ...
        raise ValueError(
            "Invalid landmarks model type. Supported models are",
            "['small', 'large']."
        )
...
def face_encodings(
    ...
    :param known_face_locations: Optional - the bounding boxes of each face if
    ...
    :param model: Optional - which model to use. "large" (default) or "small"
        which only returns 5 points but is faster.
    :return: A list of 128-dimensional face encodings (one for each face in the
        image)
    """
...
def compare_faces(known_face_encodings, face_encoding_to_check, tolerance=0.6):
    """
    Compare a list of face encodings against a candidate encoding to see if
    they match.

    :param known_face_encodings: A list of known face encodings
    :param face_encoding_to_check: A single face encoding to compare against the
        list
